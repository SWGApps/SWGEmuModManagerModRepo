...
OutputVertex main (InputVertex inputVertex)
{
	...
	// than distance.
	float  waveAtten   = saturate(1.0 - pow(camDistSqr/sqrt(128), 8));
	...
	float2 big_tc   = float2(-tcPos.x * ooShaderSize * 0.005, tcPos.z * ooShaderSize * 0.005);
	float2 tc       = gen_tc + delta_tc;
	float2 tc2       = big_tc + delta_tc;

	outputVertex.tcs0 = tc + textureScroll.xy;
	outputVertex.tcs1 = tc + textureScroll.zw;
	
	float2 manualScrollSpeed = float2(0.004, 0.004);
	
	outputVertex.tcs4 = tc2+ (manualScrollSpeed.xy * currentTime);
